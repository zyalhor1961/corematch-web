...
DECLARE
    ...
    new_org_id UUID;
BEGIN
    -- Récupérer l'utilisateur actuel (éviter le mot réservé current_user)
    ...
    RAISE NOTICE 'Utilisateur actuel: % (email: %)', current_user_id, COALESCE(user_email, 'email inconnu');
    
    -- Vérifier si l'utilisateur a déjà une organisation
    IF EXISTS (SELECT 1 FROM public.organizations WHERE admin_user_id = current_user_id) THEN
