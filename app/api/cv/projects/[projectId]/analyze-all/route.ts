...
/**
 * Analyse un candidat avec le système multi-provider orchestré (Graph-based)
 */
async function analyzeCandidateMultiProvider(
  ...
  console.log(`[🎯 Multi-Provider] Prefilter: ${enablePrefilter ? 'ENABLED' : 'DISABLED'}`);

  // Appeler le nouveau système orchestré (Graph-based)
  ...
  if (!result || !result.final_decision) {
    throw new Error('Analysis failed: Graph execution did not complete successfully');
  }
...
export async function POST(
  ...
  try {
    ...
    // Disable prefilter for auto-generated JobSpecs (too aggressive with minimal criteria)
    const isAutoGenerated = !customJobSpec && !project.job_spec_config;
    const enablePrefilter = !isAutoGenerated;
    ...
    if (customJobSpec) {
      ...
      console.log(`⚠️  Prefilter DISABLED (auto-generated JobSpec has minimal criteria)`);
    }
    ...
    for (const candidate of candidates) {
      try {
        ...
        // Analyze with multi-provider orchestrated system (Graph-based)
        const analysisResult = await analyzeCandidateMultiProvider(candidate, cvText, jobSpec, projectId, enablePrefilter);
