...
const CommandBar = ({ onMenuClick, onOpenCmd }: CommandBarProps) => {
  return (
    <header className="h-16 w-full bg-transparent z-30 flex items-center justify-between px-4 md:px-6 pointer-events-none">
      ...
      <button
        onClick={() => onOpenCmd?.()}
        className="hidden md:flex items-center w-96 px-4 py-2 bg-navy-glass/40 backdrop-blur-md border border-white/10 rounded-lg text-slate-400 hover:bg-navy-glass/60 hover:border-neon-teal/30 transition-all group pointer-events-auto"
      >
        ...
        <kbd className="ml-auto text-xs font-mono bg-white/5 px-2 py-0.5 rounded text-slate-500 group-hover:text-white transition-colors">Cmd+K</kbd>
      </button>
